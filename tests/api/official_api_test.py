...
def main():
    ...
    print("="*60)
    print(f"\n交易对: {SYMBOL}")
    print(f"测试流程: 限价买单 -> 查询当前委托 -> 市价买单 -> 市价卖单 -> 查询历史 -> 取消限价单")
    print(f"\n开始测试...")
    ...
    print(f"限价单取消: {'✅' if results.get('cancel_success') else '⚠️'}")
    
    print("\n" + "="*60)
    print("测试完成!")
    print("="*60)
    print("\n✅ 测试流程已完成:")
    print("  1. 下限价买单（95%价格，不会立即成交）")
    print("  2. 查询当前委托（确认限价单存在）")
    print("  3. 下市价买单（立即成交）")
    print("  4. 下市价卖单（立即成交，恢复账户状态）")
    print("  5. 查询历史委托和交易详情")
    print("  6. 取消限价单（清理未成交订单）")
    print("\n账户应该已经恢复到接近初始状态。")
